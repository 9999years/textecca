...
let
  ...
  cmdOutput = { cmd, name ? "command", ... }@attrs_:
    let
      inherit (pkgs) runCommandNoCCLocal;
      attrs = (removeAttrs attrs_ [ "cmd" "name" ]);
    in lib.readFile "${runCommandNoCCLocal name attrs "${cmd} > $out"}";
